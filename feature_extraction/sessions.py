...
class Session:
    ...
    def calculate_features(self):
        ...
        self.mean_inter_action_time = statistics.mean(inter_action_times)

        row = [self.num_actions, self.mean_inter_action_time,
               ...
               self.mm_mean_curvature, self.mm_sd_curvature,  # self.mm_max_curvature, self.mm_min_curvature,
               ...
               # self.mm_max_max_deviation, self.mm_min_max_deviation
               ...
               self.pc_mean_curvature, self.pc_sd_curvature,  # self.pc_max_curvature, self.pc_min_curvature,
               ...
               self.dd_mean_curvature, self.dd_sd_curvature,  # self.dd_max_curvature, self.dd_min_curvature])
    ...
    def process_mouse_move_actions(self, mm_actions):
        durations, trajectories, distances, straightness, num_points, velocities, accelerations, jerks, \
        deviations, angle_sums, curvatures = get_lists(mm_actions)
        ...
        [self.mm_mean_duration, self.mm_sd_duration, self.mm_max_duration, self.mm_min_duration] = mean_sd_max_min(
            durations)
        [self.mm_mean_trajectory, self.mm_sd_trajectory, self.mm_max_trajectory,
         self.mm_min_trajectory] = mean_sd_max_min(trajectories)
    ...
    def process_point_click_actions(self, pc_actions):
        durations, trajectories, distances, straightness, num_points, velocities, accelerations, jerks, \
        deviations, angle_sums, curvatures = get_lists(pc_actions)

        click_times, approach_times = ([] for _ in range(2))
        ...
        [self.pc_mean_duration, self.pc_sd_duration, self.pc_max_duration, self.pc_min_duration] = mean_sd_max_min(
            durations)
        [self.pc_mean_trajectory, self.pc_sd_trajectory, self.pc_max_trajectory,
         self.pc_min_trajectory] = mean_sd_max_min(trajectories)
        [self.pc_mean_distance, self.pc_sd_distance, self.pc_max_distance, self.pc_min_distance] = mean_sd_max_min(
            distances)
        [self.pc_mean_straightness, self.pc_sd_straightness, self.pc_max_straightness,
         self.pc_min_straightness] = mean_sd_max_min(straightness)
        [self.pc_mean_num_points, self.pc_sd_num_points, self.pc_max_num_points,
         self.pc_min_num_points] = mean_sd_max_min(num_points)
        [self.pc_mean_angle_sum, self.pc_sd_angle_sum, self.pc_max_angle_sum, self.pc_min_angle_sum] = mean_sd_max_min(
            angle_sums)
        ...
        [self.pc_mean_click_time, self.pc_sd_click_time, _, _] = mean_sd_max_min(click_times)
        [self.pc_mean_approach_time, self.pc_sd_approach_time, self.max_approach_time,
         self.min_approach_time] = mean_sd_max_min(approach_times)

    def process_drag_drop_actions(self, dd_actions):
        durations, trajectories, distances, straightness, num_points, velocities, accelerations, jerks, \
        deviations, angle_sums, curvatures = get_lists(dd_actions)
...
def get_lists(actions):
    durations, trajectories, distances, straightness, num_points = ([] for _ in range(5))
    velocities, accelerations, jerks, deviations, angle_sums, curvatures = ([] for _ in range(6))
    ...
    return durations, trajectories, distances, straightness, num_points, velocities, accelerations, jerks, \
           deviations, angle_sums, curvatures
